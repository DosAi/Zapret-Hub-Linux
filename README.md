...
поддерживается. Его место в переключателе занимает Happ.
...
Happ выбирается на главной странице как основной режим. Кнопка питания
подключает и отключает его через официальные deeplink-команды. Импорт
